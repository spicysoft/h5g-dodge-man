...
//      throw 'double free!';
  ...
  bytesToPixelsRoundedDown: function bytesToPixelsRoundedDown(bytes) {
    return (bytes * emscriptenMemoryProfiler.canvas.width * emscriptenMemoryProfiler.canvas.height / TOTAL_MEMORY) | 0;
  },
  ...
  bytesToPixelsRoundedUp: function bytesToPixelsRoundedUp(bytes) {
    return ((bytes * emscriptenMemoryProfiler.canvas.width * emscriptenMemoryProfiler.canvas.height + TOTAL_MEMORY - 1) / TOTAL_MEMORY) | 0;
  },
 ...
 filterCallstack: function(callstack) {
  ...
  updateUi: function updateUi() {
    ...
    var html = 'Total HEAP size: ' + self.formatBytes(TOTAL_MEMORY) + '.';
    ...
    html += "<br />" + colorBar("#6699CC") + colorBar("#003366") + colorBar("#0000FF") + "DYNAMIC memory area used: " + self.formatBytes(self.totalMemoryAllocated) + " (" + (self.totalMemoryAllocated * 100 / (TOTAL_MEMORY - DYNAMIC_BASE)).toFixed(2) + "% of all dynamic memory and unallocated heap)";
    html += "<br />Free memory: " + colorBar("#70FF70") + "DYNAMIC: " + self.formatBytes(DYNAMICTOP - DYNAMIC_BASE - self.totalMemoryAllocated) + ", " + colorBar('#FFFFFF') + 'Unallocated HEAP: ' + self.formatBytes(TOTAL_MEMORY - DYNAMICTOP) + " (" + ((TOTAL_MEMORY - DYNAMIC_BASE - self.totalMemoryAllocated) * 100 / (TOTAL_MEMORY - DYNAMIC_BASE)).toFixed(2) + "% of all dynamic memory and unallocated heap)";
